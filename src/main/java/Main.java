...
public class Main {
		...
		try (ServerSocket serverSocket = new ServerSocket(8080);) {
			while (run) {
				...
				if (inputLine.startsWith("execute")) {
					System.out.print("\nLoader completed, sleeping 1 second then scanning table...");	
					Thread.sleep(1000);
					...
					elapsed = System.currentTimeMillis() - elapsed - time + 1000;
					outputLine = String.format("%d of %d items scanned in %dms.  %sReplication lag: %dms",
							total - (numTransactions * (iteration - 1)), numTransactions, time, elapsed > 2000 ? "ERROR: " : "", elapsed);

					// send the result to the source region and log to console.
					out.println(outputLine);
					System.out.println();
					System.out.println(String.format("%s:  %s", iso.format(Calendar.getInstance().getTime()), outputLine));
					System.out.println();
					break;
				}
			}
	...
	private static void send(String hostName, int iteration) {
		...
		try (Socket socket = new Socket(hostName, 8080);
			...
			System.out.println(String.format("%s:  %s", iso.format(Calendar.getInstance().getTime()), fromServer));
